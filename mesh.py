...
def compute_all_cell_face_normals_cuda(cell_face_idx, cell_node_idx, node_coordinates, normals_device):
    ...
    if cell_id < n_cells and face_id < n_faces:
        ...
        node_indices = cuda.local.array(4, dtype=nb.uint64)  # Maximum 4 nodes per face
...
def compute_cardinal_index(cardinals, cell_face_idx, cell_node_idx, node_coordinates):
    ...
    cardinal_idx_device = cuda.device_array((cell_node_idx.shape[0], cell_face_idx.shape[0]), dtype=np.uint64)
    ...
    def write_ffa_connectivity(self, file_name: str):
        ...
        for key in self.boundaries:
            ...
            data = np.array([[face.neighbours[0].index, face.neighbours[0].neighbours.index(face)]
                             for face in self.boundaries[key].faces], dtype=np.uint64)
        ...
        data = np.array([[neighbour.index if isinstance(neighbour, Cell) else cell.index
                         ...
                         for cell in self.cells], dtype=np.uint64)
...
import ffa
from lbm.mesh.node import Node
from lbm.mesh.cell import Cell, compute_normal
from lbm.mesh.face import Face
from lbm.mesh.boundary import Boundary
...
@nb.njit
def compute_all_cell_face_normals(cell_face_idx, cell_node_idx, node_coordinates):
    # Each cell has a number of faces, lets compute the normal for each face
    # Each face has a number of nodes, defined in cell_face_idx
    dim = node_coordinates.shape[1]
    n_cells = cell_node_idx.shape[0]
    n_face = cell_face_idx.shape[0]
    normal = np.empty((n_cells, n_face * dim))
    for i in range(n_cells):
        for j in range(n_face):
            idx = cell_node_idx[i][cell_face_idx[j]]
            normal[i, dim*j:dim*(j+1)] = compute_normal(node_coordinates[idx])
    return normal


def compute_cardinal_index(cardinals: np.ndarray, cell_face_idx: np.ndarray,
                           cell_node_idx: np.ndarray, node_coordinates: np.ndarray):
    normal = compute_all_cell_face_normals(cell_face_idx, cell_node_idx, node_coordinates)
    dim = node_coordinates.shape[1]
    n_cells = cell_node_idx.shape[0]
    n_face = cell_face_idx.shape[0]
    cardinal_idx = np.zeros((n_cells, n_face), dtype=np.uint64)
    for i in range(n_cells):
        for j in range(n_face):
            cardinal_idx[i, j] = (normal[i, dim*j:dim*(j+1)] == cardinals).all(axis=1).nonzero()[0][0]
    return cardinal_idx
